...
var processTimeoutMS = args[2] || 1000 * 60 * 60;
var generationTimeoutMS = args[1] || 30000;
system.stdout.write(JSON.stringify({log: 'Generation timeout: ' + generationTimeoutMS + '; Process timeout: ' + processTimeoutMS}));
// Error handler
function exit(error) {
    var message
...
var processTimeoutFn = function () {
    exit('Exit timeout')
};

var processTimeout = setTimeout(processTimeoutFn, processTimeoutMS);
var pageGenerationTimeout = null;
...
setInterval(function () {
    ...
    processTimeout = setTimeout(processTimeoutFn, processTimeoutMS);
    pageGenerationTimeout = setTimeout(function () {
        exit('Page generation timeout');
    }, generationTimeoutMS);
...
page.onLoadFinished = function (status) {
    ...
    // This is not a type, this is a workaround to clipped printing with 2.1+ phantomjs.
    // If someone ever remove this, test ALL possible printing combinations(as attachment, from desktop app, multiple pages, etc)
    ...
    clearTimeout(pageGenerationTimeout);
    //exit(null)
};
